...
def agent_instruction_snippet(db_dir: str) -> str:
    ...
    return (
        ...
        "Choose the right tool based on query type — do **not** default to grep for everything,"
        " but do **not** use cosk as a substitute for grep either:\n\n"
        "- **Concept / semantic queries** (\"find code related to X\", \"how is Y implemented\")"
        " → use cosk tools\n"
        "- **Exact name / substring / pattern matching** (\"find all symbols containing string X\")"
        " → use **grep**\n\n"
        "**Do not use `cosk_semantic_search` for exact name lookups** — it is a vector similarity"
        " search, not a name filter. For name-pattern searches, grep is more precise and"
        " token-efficient.\n\n"
        "### When to use\n"
