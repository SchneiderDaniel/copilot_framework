...
**🛑 CRITICAL: Step 4 (finalize.py) MUST be executed by YOU directly using the `execute` tool. The `--comment-file` argument is MANDATORY — always write your design to a file first and pass it. Never call finalize.py without `--comment-file`. If finalize.py fails, report the error immediately.**
